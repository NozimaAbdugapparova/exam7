...
const API_URL    = `${BASE}/api/teachers/all/archived`;
...
function Avatar({ src, firstName, lastName }) {
  ...
  return <img src={src} alt={full} onError={() => setErr(true)}
    className="w-8 h-8 rounded-full object-cover shrink-0 border border-gray-200" />;
}

const COLUMNS = ["Rasm", "Ism familiyasi", "Email", "Telefon", "Manzil", "Amallar"];
...
export default function ArchivedTeachers() {
  ...
  const toggleAll   = () => setSelected(allSelected ? [] : paginated.map((t) => t.id));
      ...
      <div className="px-4 pt-4 pb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">O'chirilgan o'qituvchilar ro'yxati</p>
          </div>
        </div>
      </div>
      ...
      <div className="mx-4 flex-1 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-100">
          <button disabled={selected.length === 0}
            ...
            <Trash2 size={13} />Delete {selected.length > 0 && `(${selected.length})`}
          </button>
        </div>
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="flex-1 flex items-center justify-center py-16">
            <span className="text-xs text-gray-400">Arxivda o'qituvchilar yo'q</span>
          </div>
        )}
          ...
          <div className="flex-1 overflow-auto">
            <table className="w-full text-left">
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {paginated.map((t) => (
                  <tr key={t.id} className={`border-b border-gray-100 transition-colors ${selected.includes(t.id) ? "bg-blue-50/40" : "hover:bg-gray-50/60"}`}>
                    ...
                    <td className="px-3 py-2.5 w-12">
                      <Avatar src={t.photo ? `${PHOTO_BASE}${t.photo}` : null} firstName={t.first_name} lastName={t.last_name} />
                    </td>
                    <td className="px-3 py-2.5 text-xs font-semibold text-gray-800">{t.first_name} {t.last_name}</td>
                    <td className="px-3 py-2.5 text-xs text-gray-500">{t.email   || "—"}</td>
                    <td className="px-3 py-2.5 text-xs text-gray-500">{t.phone   || "—"}</td>
                    <td className="px-3 py-2.5 text-xs text-gray-500">{t.address || "—"}</td>
