...
import { Search, SlidersHorizontal, Download, Plus, Archive, Loader2, Minus, Eye, Trash2, Pencil } from "lucide-react";
import AddStudentDrawer from "../components/AddStudentDrawer";

const API_URL = "http://localhost:3000/api/students/all";
...
function StudentRow({ student, selected, onSelect }) {
  ...
  const avatarSrc = student.photo ? `${PHOTO_BASE_URL}${student.photo}` : null;
    ...
    <tr className={`border-b border-gray-100 transition-colors ${selected ? "bg-blue-50/40" : "hover:bg-gray-50/60"}`}>
      ...
      <td className="px-3 py-2.5">
        <div className="flex items-center gap-0.5">
          <button className="p-1.5 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors"><Minus size={12} /></button>
          <button className="p-1.5 rounded-md text-gray-400 hover:text-green-600 hover:bg-green-50 transition-colors"><Plus size={12} /></button>
          <button className="p-1.5 rounded-md text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors"><Eye size={13} /></button>
          <button className="p-1.5 rounded-md text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 size={13} /></button>
          <button className="p-1.5 rounded-md text-gray-400 hover:text-purple-600 hover:bg-purple-50 transition-colors"><Pencil size={13} /></button>
        </div>
      </td>
    </tr>
  );
    ...
    <div className="flex flex-col min-h-screen bg-[#f4f5f7]">
      ...
      <div className="px-4 mb-3 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <button className="flex items-center gap-1 px-3 py-1.5 text-xs text-gray-600 border border-gray-200 bg-white rounded-lg hover:bg-gray-50">
      ...
      <div className="mx-4 flex-1 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-100">
          ...
          <button className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-red-500 border border-red-200 rounded-lg hover:bg-red-50">
        ...
        {error && !loading && (
          <div className="flex-1 flex flex-col items-center justify-center py-16 gap-2">
            ...
            <button onClick={() => window.location.reload()} className="text-xs text-blue-600 underline">Qayta urinish</button>
          </div>
        )}
